...
/*
   Stampa del rendiconto 'Stato patrimoniale' secondo nuovi schemi di bilancio per ASD.
...
function stampaReportControllo(banDoc, paramReport) {
   ...
   bReportControllo.excludeEntries();
...
function printRendicontoModA_Attivo(banDoc, report, userParam, bReport) {
   ...
   /* ACIV4 */
   printRow(userParam, bReport, table, "ACIV4", "description-groups", "amount-groups");
   /* tot ACIV */
...
function setCss(banDoc, repStyleObj, userParam) {
   ...
   if (userParam.stampareportcontrollo) {
      ...
      file = Banana.IO.getLocalFile("file:script/ch.banana.it.extension.asd.statopatrimoniale.css");
   }
