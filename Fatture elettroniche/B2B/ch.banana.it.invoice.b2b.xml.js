...
//
// @id = ch.banana.it.invoice.b2b.xml.js
// @api = 1.0
// @pubdate = 2018-07-30
// @publisher = Banana.ch SA
// @description = Electronic invoice B2B in xml format version 1.2.1
// @description.it = Fattura elettronica B2B in formato xml versione 1.2.1
// @doctype = *
// @includejs = 
...
function settingsDialog() {
  ...
  if (typeof (Banana.Ui.openPropertyEditor) !== 'undefined') {
    ...
    param.incremental = Banana.Ui.getInt('Settings', 'Incremental', param.incremental);
...
// function checkErrors(jsonInvoice)
// {
//   if (typeof (jsonInvoice) === 'string') 
//     jsonInvoice = JSON.parse(jsonInvoice)
  
//   if (!jsonInvoice.customer_info.business_name || (!jsonInvoice.customer_info.first_name && !jsonInvoice.customer_info.last_name))
//     addMessage('Missing client organization name, or first name and last name', null)
...
FatturaElettronica.prototype.createInstance = function (xmlDocument, jsonInvoice) {
  ...
  nodeIdPaese.addTextNode(this.invoiceObj.supplier_info.country);
  var nodeIdCodice = nodeIdTrasmittente.addElement("IdCodice");
  nodeIdCodice.addTextNode((this.invoiceObj.supplier_info.country == 'IT' ? this.invoiceObj.supplier_info.fiscal_number.substring(2) : this.invoiceObj.supplier_info.fiscal_number));
  ...
  //Banana.console.debug(JSON.parse(this.datiContribuente).tipoContribuente);
...
FatturaElettronica.prototype.saveData = function (output) {
  ...
  if (this.invoiceObj.supplier_info.country_code === 'IT')
    fileName += this.invoiceObj.supplier_info.fiscal_number;
  else
    fileName += this.invoiceObj.supplier_info.fiscal_number;
